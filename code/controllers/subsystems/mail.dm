...
/datum/controller/subsystem/mail/proc/create_mail()
	...
	for(var/mob/living/carbon/human/player_human in GLOB.player_list)
		if(player_human.stat != DEAD && player_human.client && player_human.client.inactivity <= 10 MINUTES && !(player_human.job in list(JOB_OUTSIDER, JOB_OUTSIDER_ROBOT, JOB_ANOMALY, JOB_VR)) && !player_is_antag(player_human.mind) && !isbelly(player_human.loc)) // Only alive, active and NT employeers should be getting mail.
